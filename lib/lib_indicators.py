...
def pearson_correlation(y_true, y_pred, time_frequency=np.array([])):
    ...
    if time_frequency.any() == 0:
        ...
        ssd_true = np.sqrt((1 / (n - 1)) * np.nansum(time_frequency*(y_true - y_true_mean) ** 2))  # sample standard deviation
...
def spearman_correlation_coded(y_true, y_pred):
    ...
    n = len(y_true)
    rs = 1. - ( 6 * np.nansum(diff**2) / (n * (n**2 - 1)) )
    return rs
...
def nbias(y_true, y_pred, time_frequency=np.array([])):
    ...
    nbias = (y_pred_mean - y_true_mean) / y_true_mean #y_pred_mean
    #nbias = (y_true_mean - y_pred_mean) / y_pred_mean
    return nbias
...
def absnbias(y_true, y_pred, time_frequency=np.array([])):
    ...
    #nbias = abs((y_true_mean - y_pred_mean) / y_pred_mean)
    return nbias
...
def main():
    ...
    #time_frequency_test = np.array([0.1, 0.1, 0.2, 0.2, 0.4])
