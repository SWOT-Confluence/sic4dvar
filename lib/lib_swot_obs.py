...
swot_common_q_b_decimal_labels = {
    "classification_qual_suspect": 2,
    "geolocation_qual_suspect": 4,
    "water_fraction_suspect": 8,
    "bright_land": 128,
    "few_area_observations": 1024,
    "few_wse_observations": 2048,
    "far_range_suspect": 8192,
    "near_range_suspect": 16384,
    "classification_qual_degraded": 262144,
    "geolocation_qual_degraded": 524288,
    "lake_flagged": 4194304,
    "no_area_observations": 67108864,
    "no_wse_observations": 134217728,
    "no_observations": 268435456
}

swot_q_b_decimal_labels = {
    "node": {
        "sig0_qual_suspect": 1,
        "blocking_width_suspect": 16,
        "few_sig0_observations": 512,
        "wse_outlier": 8388608,
        "wse_bad": 16777216,
        "no_sig0_observations": 33554432
    },
    "reach": {
        "partially observed": 32768,
        "below min fit points": 33554432
    }
}
...
def filter_swot_obs_on_quality(swot_dict_arrays, val_swot_q_flag):
    ...
    for q_var, q_val in val_swot_q_flag["node"].items():
        if q_var == "xtrk_dist_max": var_mask = np.greater(np.abs(swot_dict_arrays["node"]["xtrk_dist"]), q_val).filled(np.nan)
        elif  q_var == "xtrk_dist_min": var_mask = np.less(np.abs(swot_dict_arrays["node"]["xtrk_dist"]), q_val).filled(np.nan)
        elif  q_var == "ice_clim_f": var_mask =  ~np.isin(swot_dict_arrays["node"]["ice_clim_f"], q_val)
        elif  q_var == "dark_frac_max": var_mask = np.greater(swot_dict_arrays["node"]["dark_frac"], q_val).filled(np.nan)
        elif  q_var == "n_good_pix_min": var_mask = np.less(swot_dict_arrays["node"]["n_good_pix"], q_val).filled(np.nan)
        elif  q_var == "layovr_val_min": var_mask = np.less(swot_dict_arrays["node"]["layovr_val"], q_val).filled(np.nan)
        elif  q_var == "layovr_val_max": var_mask = np.greater(swot_dict_arrays["node"]["layovr_val"], q_val).filled(np.nan)
        elif  q_var == "node_dist_max": var_mask = np.greater(swot_dict_arrays["node"]["node_dist"], q_val).filled(np.nan)
        elif  q_var == "wse_u_max": var_mask = np.greater(swot_dict_arrays["node"]["wse_u"], q_val).filled(np.nan)
        elif  q_var == "width_u_max": var_mask = np.greater(swot_dict_arrays["node"]["width_u"], q_val).filled(np.nan)
        elif  q_var == "flow_angle_min": var_mask = np.less(swot_dict_arrays["node"]["flow_angle"], q_val).filled(np.nan)
        elif  q_var == "flow_angle_max": var_mask = np.greater(swot_dict_arrays["node"]["flow_angle"], q_val).filled(np.nan)
        elif  q_var == "node_q": var_mask =  ~np.isin(swot_dict_arrays["node"]["node_q"], q_val)
        elif  q_var == "node_q_b": var_mask =  filter_q_b(swot_dict_arrays["node"]["node_q_b"], q_val)
        elif  q_var == "xovr_cal_q": var_mask =  ~np.isin(swot_dict_arrays["node"]["xovr_cal_q"], q_val)
        elif  q_var == "wse_r_u_max": var_mask =  np.greater(swot_dict_arrays["node"]["wse_r_u"], q_val).filled(np.nan)
        else:
        ...
        print(f"Filtering SWOT Obs node with var {q_var} with values {q_val} : "
              f"{np.sum(var_mask)} values to remove, {np.sum(~node_level_values_to_remove_mask)} left")
        
    for q_var, q_val in val_swot_q_flag["reach"].items():
        if q_var == "obs_frac_n_min": var_mask = np.less(swot_dict_arrays["reach"]["obs_frac_n"], q_val).filled(np.nan)
        elif  q_var == "ice_clim_f": var_mask =  ~np.isin(swot_dict_arrays["reach"]["ice_clim_f"], q_val)
        elif q_var == "partial_f_max": var_mask = np.greater(swot_dict_arrays["reach"]["partial_f"], q_val).filled(np.nan)
        elif q_var == "slope_r_u_max": var_mask = np.greater(swot_dict_arrays["reach"]["slope_r_u"], q_val).filled(np.nan)
        elif q_var == "slope2_r_u_max": var_mask = np.greater(swot_dict_arrays["reach"]["slope2_r_u"], q_val).filled(np.nan)
        elif q_var == "reach_q_b":
            var_mask = filter_q_b(swot_dict_arrays["reach"]["reach_q_b"], q_val)
        elif  q_var == "dark_frac_max": var_mask = np.greater(swot_dict_arrays["reach"]["dark_frac"], q_val).filled(np.nan)
        elif  q_var == "width_u_max": var_mask = np.greater(swot_dict_arrays["reach"]["width_u"], q_val).filled(np.nan)
        elif  q_var == "wse_r_u_max": var_mask =  np.greater(swot_dict_arrays["reach"]["wse_r_u"], q_val).filled(np.nan)
        elif  q_var == "wse_u_max": var_mask = np.greater(swot_dict_arrays["reach"]["wse_u"], q_val).filled(np.nan)
        elif q_var == "xtrk_dist_max": var_mask = np.greater(np.abs(swot_dict_arrays["reach"]["xtrk_dist"]), q_val).filled(np.nan)
        elif  q_var == "xtrk_dist_min": var_mask = np.less(np.abs(swot_dict_arrays["reach"]["xtrk_dist"]), q_val).filled(np.nan)
        elif  q_var == "xovr_cal_q": var_mask =  ~np.isin(swot_dict_arrays["reach"]["xovr_cal_q"], q_val)

        #SWORD
        elif q_var == "reach_length_min":
            #var_mask = np.tile(swot_dict_arrays["reach"][q_var].filled(np.nan) < q_val, (1, swot_dict_arrays["nt"]))
        ...
        print(f"Filtering SWOT Obs reach with var {q_var} with values {q_val} : "
              f"{np.sum(var_mask)} values to remove, {np.sum(~node_level_values_to_remove_mask)} left")
...
def get_flag_dict_from_config(config):
    ...
    val_swot_q_flag = {
        "node": val_swot_q_flag_node,
        "reach": val_swot_q_flag_reach,
    }
...
def main():
    q_b_array = np.array([4.09620000e+04, 4.09700000e+04, 5.24290000e+05, 5.24298000e+05, 5.24302000e+05, 5.26346000e+05, 5.26350000e+05,
                 5.40682000e+05, 5.42730000e+05, 5.57066000e+05, 5.59114000e+05, 5.65248000e+05, 5.65254000e+05, 5.65258000e+05,
                 5.67296000e+05, 5.67298000e+05, 5.67302000e+05, 5.67304000e+05, 5.67306000e+05, 5.73450000e+05, 5.75498000e+05,
                 5.76522000e+05, 8.04874000e+05, 4.69762048e+08, 5.03349248e+08])
