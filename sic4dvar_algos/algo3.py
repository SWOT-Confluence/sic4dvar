...
def algo3(apr_array, Qwbm, params, SLOPEM1, valid_output, node_z,
    node_z_input, node_z_ini, node_x, last_node_for_integral,
    bathymetry_array, param_dict, reach_id='', reach_t=[], bb=9999.0,
    reliability=''):
    ...
    kDim = params.kDim
    q_pdf_table = calc.betad(int(param_dict['q_parametrization']), kDim,
        shapeP[0][0], shapeP[0][1])
    ...
    if param_dict['gnuplot_saving']:
        ...
        gnuplot_save_tables(q_pdf_table, zb_pdf_table, km_pdf_table,
            output_dir.joinpath('tables'), 1)
    temp = []
    SS1_array = []
    ...
    for k in range(LSM):
        ...
        for t in range(1, node_z[0].shape[0] - 1):
            SLOPEM2 += [0.25 * SLOPEM1[t - 1] + 0.5 * SLOPEM1[t] + 0.25 *
                SLOPEM1[t + 1]]
    ...
    for i2 in range(1, I2m + 1):
        ...
        for t in range(node_z[0].shape[0]):
            ...
            for n in range(last_node_for_integral):
                ...
                if Wmin == 0:
                    Wmin = 1.0
                if t == 0:
                    Zb[n] = node_yr[n][0] + Zb2 + (i2 - 1) * dZb * (Wmean /
                        Wmin)
                Z1 = node_yr[n][0]
                W1 = node_xr[n][0]
                if Zmin != Z1:
                ...
                if n == 0:
                    ...
                    if QM0 > 0 and QMi2i3 > 0:
                        SS1 += abs(node_x[n] - node_x[n - 1]) * (math.pow(
                            QM0, -2) + math.pow(QMi2i3, -2)) / 2.0
                    QM0 = QMi2i3
        ...
        if param_dict['gnuplot_saving']:
            ...
            gnuplot_save_slope(temp_ss1_array, times2, output_dir.joinpath(
                'integral'))
        SS1_array = []
        ...
        for i3 in range(1, I3m + 1):
            ...
            Q_pdf = interp_pdf_tables(kDim - 1, theta * kDim,
                q_positional_arg, q_pdf_table)
            ...
            if params.V32:
                ...
                if theta < 0.0:
                    Qtrial = 0.0
                QMT_array[i2 - 1, :] = QMT_const_array[i2 - 1, :
                    ] / QMEAN_array[i2 - 1] * (Qtrial * QM2 + (1.0 - Qtrial
                    ) * QM1)
            else:
                QMT_array[i2 - 1, :] = QMT_const_array[i2 - 1, :
                    ] * SS1 / QMEAN_array[i2 - 1]
            ...
            if params.a31_early_stop:
                ...
                if trial_ps0 / (trialps0_max + 1e-16
                    ) < 0.001 and trial_ps0 < trialps1_max and i3 > 0:
                    break
            if params.V32:
                NBS = len(node_z[:, :]) - 1
                icost = 0.0
                ...
                for ij in range(0, TDIM):
                    if (Zb_pdf > 0.0 and Km_pdf > 0.0 and Q_pdf > 0.0 and 
                        Weight[iis] > 0.0):
                        ...
                        if not node_z_ini[NBS, ij]:
                            ...
                            if params.useEXT:
                                ...
                                Z_SS, A_SS, P_SS, R_SS, cost = (calc.
                                    compute_steady_state(NBS, NBT, node_xr,
                                    node_yr, node_z_ini[:, ij], node_x, Zb,
                                    Kmi3, [Q_sample[iis, ij]]))
                            else:
                                ...
                                Z_SS, A_SS, P_SS, R_SS, cost = (calc.
                                    ...
                                    Kmi3, [Q_sample[iis, ij]]))
                            icost += cost
                if (Zb_pdf > 0.0 and Km_pdf > 0.0 and Q_pdf > 0.0 and icost >
                    0.0):
    ...
    if Nbeta == 0:
        logging.info(
            'Nbeta = 0.0, discharge will be incorrect ! Valid set of solutions is empty, observations and qwbm are not consistent !'
            )
    ...
    if params.V32:
        ...
        lc0 += [lcmax]
        id = 0
        distp, LH_pdf, QPost = calc.likelihood_2(TDIM, nsobs, lc0[id],
            Q_sample, QM, cost_all, Weight)
        distp0 += [distp]
        while distp < params.thres and distp >= distp0[id
            ] and not calc.check_na(distp):
            id += 1
            lc0 += [lc0[id - 1] + 1]
            distp, LH_pdf, QPost = calc.likelihood_2(TDIM, nsobs, lc0[id],
                Q_sample, QM, cost_all, Weight)
        ...
        lc = lc0[ilc]
        distp, LH_pdf, QPost = calc.likelihood_2(TDIM, nsobs, lc, Q_sample,
            QM, cost_all, Weight)
        QM = QPost
    for t in range(node_z[0].shape[0]):
