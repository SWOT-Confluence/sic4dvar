...
def split_freq_datetime_string(freq_datetime: str) -> Tuple[int | None, str]:
    if not freq_datetime:
        freq_datetime = "1s"
    match = re.match(r"([0-9]+)([a-z]+)", freq_datetime, re.I)
    if match:
        items = match.groups()
...
def dt_as_pd_datetime(
        dt: float | int | datetime | np.datetime64 | pd.Timestamp,
        ref_datetime: datetime | np.datetime64 | pd.Timestamp = sic_def.def_ref_datetime,
        pd_freq: str | None = None,
) -> pd.Timestamp:
...
def compute_teta(
        ...
        y0: float = None,
        degrees: bool = False,
        raise_undetermined_form_error: bool = True,
        float_atol: float = sic_def.def_float_atol,
    ...
    return compute_slope(
        x1=x1, y1=y1, x2=x2, y2=y2, x0=x0, y0=y0, as_angle=ang, float_atol=float_atol,
        raise_undetermined_form_error=raise_undetermined_form_error,
        force_positive=force_positive, clean_run=clean_run,
    )
...
def compute_slope(
        ...
        x0: float = None,
        y0: float = None,
        as_angle: None | str = None,
        raise_zero_division_error: bool = False,
        raise_undetermined_form_error: bool = True,
        float_atol: float = sic_def.def_float_atol,
        force_positive: bool = False,
        clean_run: bool = False,
...
if __name__ == '__main__':
    cs_w_test_array = np.array([5.15, 112., 113.66, 120.72, 466.78, 492.04, 494., 512.])
    ...
    print(compute_slope(x1=cs_w_test_array[0], y1=cs_z_test_array[0], x2=cs_w_test_array[1], y2=cs_z_test_array[1],
                        as_angle="deg"))
