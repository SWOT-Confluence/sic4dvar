...
def helper_plot_lines(
        xs: Iterable,
        ys: Iterable,
        show: bool = True,
        line_styles: Tuple = ('-',),
        line_widths: Tuple = (1.5,),
        line_colors: Tuple = ('black',),
        line_labels: Tuple = ('main',),
        ...
        legend_bbox_to_anchor: tuple = (1.15, 1.02),
        legend_ncols: int = 1,
    ...
    if add_text_bottom_right:
        #         x     y
        ax.text(-0.05, -0.12, add_text_bottom_right,
                verticalalignment='top', horizontalalignment='left',
                transform=ax.transAxes)

    if add_text_bottom_left:
        #         x     y
        ax.text(0.7, -0.12, add_text_bottom_left,
                verticalalignment='top', horizontalalignment='left',
                transform=ax.transAxes)
...
def helper_plot_scatter(
        xs: Iterable,
        ys: Iterable,
        show: bool = True,
        marker_styles: Tuple = ('o'),
        marker_sizes: Tuple = (5.,),
        marker_fill_colors: Tuple = ('gray',),
        marker_edge_colors: Tuple = ('gray',),
        marker_transparency: Tuple = (10.,),
        marker_labels: Tuple = ('main',),
        title: str = '',
        x_axis_title: str = '',
        y_axis_title: str = '',
        x_lim: Iterable | None = None,
    ...
    if add_identity_line:
        x_min, x_max = np.nanquantile(np.array(xs), [0., 1.])
        y_min, y_max = np.nanquantile(np.array(ys), [0., 1.])
    ...
    for n, (x, y) in enumerate(zip(xs, ys)):
        ...
        if marker_transparency is not None:
            plot_kwargs["alpha"] = (100. - marker_transparency[n]) / 100.
