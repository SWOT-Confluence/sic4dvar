...
from sic4dvar_functions.sic4dvar_helper_functions import get_input_data, close_logger, set_logger, get_reach_dataset, enable_prints, write_output
...
def seq_run(j, param_dict):
    i = j
    reach_dict = get_reach_dataset(param_dict['json_path'], i, param_dict)
    log_path = param_dict['log_dir'].joinpath('sic4dvar_' + str(reach_dict[
        'reach_id']) + '.log')
    set_logger(logging.INFO, log_path)
    logging.info('Run reach %d (the %d th)' % (reach_dict['reach_id'], i + 1))
    ...
    if type(input_data) != int:
        ...
        if processed.output['valid']:
            ...
            if 'station_q' in input_data.keys(
                ) and 'station_date' in input_data.keys():
                logging.info('Indicators computation')
                station_df = pd.DataFrame({'station_q': input_data[
                    'station_q'], 'station_date': input_data['station_date']})
                ...
                sic4dvar_date = daynum_to_date(processed.output['time'][
                    valid_idx], '2000-01-01')
                sic4dvar_df = pd.DataFrame({'sic4dvar_q': processed.output[
                    'q_algo31'][valid_idx], 'sic4dvar_date': sic4dvar_date})
                data_df = sic4dvar_df.merge(station_df, how='inner',
                    left_on='sic4dvar_date', right_on='station_date')
                if len(data_df) > 1:
                    indicators = {'reach_id': reach_dict['reach_id'],
                        'nb_dates': len(data_df), 'width': input_data[
                        'reach_w_mean'][0], 'reach_length': input_data[
                        'reach_length'][0], 'slope': input_data[
                        'reach_slope'][0], 'obs_filtering_ratio': processed
                        .output['obs_filtering_ratio']}
                    indicators.update(compute_all_indicators_from_predict_true
                        (np.array(data_df['station_q']).astype('float'), np
                        .array(data_df['sic4dvar_q']).astype('float')))
                    logging.info(
                        f"Indicators name : {';'.join(list(indicators.keys()))}"
                        )
                    logging.info(
                        f"Indicators values : {';'.join([str(e) for e in indicators.values()])}"
                        )
            ...
            if processed.reliability == 'unreliable':
                pass
            write_output(param_dict['output_dir'], param_dict, reach_dict[
                'reach_id'], processed.output, algo5_results=processed.
                algo5_results, bb=processed.bb, reliability=processed.
                reliability)
        else:
            logging.info('Results are INVALID!')
            if param_dict['aws']:
                logging.info('Writing results to output directory for AWS.')
                write_output(param_dict['output_dir'], param_dict,
                    reach_dict['reach_id'], processed.output, algo5_results
                    =processed.algo5_results, bb=processed.bb, reliability=
                    processed.reliability)
    close_logger()


def sic4dvar_seq_run(param_dict):
    logging.info('Running SIC4DVAR sequentially on json_path one by one')
    t0 = datetime.datetime.utcnow()
    with open(param_dict['json_path']) as jsonfile:
        dataj = json.load(jsonfile)
        logging.info('Number of reaches is : ' + str(len(dataj)))
    if params.create_tables:
        reach_dict = get_reach_dataset(param_dict['json_path'], 0, param_dict)
    ...
    if param_dict['aws']:
        ...
        down = 0
        upper = len(dataj)
    logging.info('down,upper: %d %d' % (down, upper))
    close_logger()
    if param_dict['flag_parallel']:
        args = [(seq_run, (j, param_dict)) for j in range(down, upper)]
        with ProcessPoolExecutor(max_workers=params.max_cores) as executor:
            list(executor.map(execute_function_old, args))
    else:
        for j in range(down, upper):
            seq_run(j, param_dict)
    t1 = datetime.datetime.utcnow()
    enable_prints()


def sic4dvar_set_run(param_dict, reach_dict):
    logging.info('Running SIC4DVAR sequentially on sets')
    logging.info('Number of sets is : %d ' % len(reach_dict))
    t0 = datetime.datetime.utcnow()
    ...
    for i_set in range(down, upper):
        ...
        set_logger(logging.INFO, log_path)
        logging.info('Processing set %d over %d with %d reaches: %s' % (
            i_set, upper, len(reach_list), ' '.join(reach_list)))
        run_flag = True
        processed = []
        temp2 = {}
        concat_dict = {}
        dim_n = []
        dim_t = []
        keys = ['node_w', 'node_z', 'node_s', 'node_dA', 'reach_w',
            'reach_z', 'reach_s', 'reach_dA', 'node_t', 'reach_qwbm',
            'node_length', 'dist_out', 'reach_id', 'reach_t', 'facc',
            'q_monthly_mean']
        log_name = 'sic4dvar_'
        reaches_ids = []
        ...
        for j in range(0, nbr_reach):
            if nbr_reach > 1:
                if str(reach_dict[i_set][j]['reach_id'])[-1] == '1':
                    input_data, flag_dict = get_input_data(param_dict,
                        reach_dict[i_set][j])
                    processed.append(algorithms(input_data, flag_dict,
                        param_dict))
                    reaches_ids.append(reach_dict[i_set][j]['reach_id'])
                else:
                    logging.info(
                        f"Reach {reach_dict[i_set][j]['reach_id']} not processed because reach type != 1"
                        )
            else:
                
                if 'reach_id' in reach_dict[i_set]:
                    if str(reach_dict[i_set]['reach_id'])[-1] == '1':
                        input_data, flag_dict = get_input_data(param_dict,
                            reach_dict[i_set])
                        processed.append(algorithms(input_data, flag_dict,
                            param_dict))
                        log_name = log_name + str(reach_dict[i_set]['reach_id']
                            ) + '_'
                    ...
                    if str(reach_dict[i_set][0]['reach_id'])[-1] == '1':
                        ...
                        processed.append(algorithms(input_data, flag_dict,
                            param_dict))
                        log_name = log_name + str(reach_dict[i_set][0][
                            'reach_id']) + '_'
            ...
            for key in keys:
                ...
                for i in range(0, nbr_reach):
                    ...
                    if 'valid_min_z' in processed[i].input_data:
                        params.valid_min_z = processed[i].input_data[
                            'valid_min_z']
                        params.valid_min_dA = processed[i].input_data[
                            'valid_min_dA']
                    else:
                        pass
                    if key in processed[i].input_data:
                        ...
                        """
                        if key == "reach_t":
                            tmp_t.append(processed[i].input_data[key])
                            pass
                       """
                    else:
                        if key == 'node_t':
                            dim_n.append(processed[i].input_data[
                                'node_length'].shape[0])
                            nb_lines += processed[i].input_data['node_length'
                                ].shape[0]
                        empty_array = np.full((processed[i].input_data[
                            'node_length'].shape[0], get_dim_t), np.nan)
        ...
        if run_flag:
            ...
            for j in range(1, Unique_T.shape[0]):
                if Unique_T[j] >= Generic_T[ig]:
                    index = np.where(Unique_T[j:] - Generic_T[ig] >= params
                        .DT_obs)
                    index = index[0]
            ...
            keys4 = ['dist_out', 'node_length', 'reach_qwbm', 'facc',
                'q_monthly_mean']
            for key in keys2:
            ...
            for nb_process in range(0, len(temp2['node_t'])):
                ...
                for key in keys_tmp:
                    if key == 'q_monthly_mean':
                        concat_dict[key].append(np.array(temp2[key][
                            nb_process]))
                    else:
                        if np.array(temp2[key][nb_process]).size > 1:
                            concat_dict[key] = np.sort(np.hstack((
                                concat_dict[key], np.flipud(temp2[key][
                                nb_process]))))
                        else:
                            concat_dict[key] = np.sort(np.hstack((
                                concat_dict[key], temp2[key][nb_process])))
                        if params.densification:
                            if key == 'dist_out':
                                if np.array(temp2[key][nb_process]).size > 1:
                                    concat_dict['dist_out_all'].append(temp2
                                        [key][nb_process])
                                else:
                                    concat_dict['dist_out_all'].append(temp2
                                        [key][nb_process])
                            if key == 'node_length':
                                if np.array(temp2[key][nb_process]).size > 1:
                                    concat_dict['node_length_all'].append(temp2
                                        [key][nb_process])
                                else:
                                    concat_dict['node_length_all'].append(temp2
                                        [key][nb_process])
                ...
                for nb_process_tmp in range(0, len(temp2['node_t'])):
                    for key in keys_tmp:
                        if nb_process == 0:
                            ...
                            concat_dict[key] = np.hstack((concat_dict[key],
                                temp2[key][nb_process_tmp]))
                concat_dict['reach_qwbm'] = np.nanmean(concat_dict[
                    'reach_qwbm'])
                masked_data = np.ma.masked_values(np.array([concat_dict[
                    'reach_qwbm']]), value=-9999.0)
                concat_dict['reach_qwbm'] = masked_data
                concat_dict['facc'] = np.nanmean(concat_dict['facc'])
                masked_data2 = np.ma.masked_values(np.array([concat_dict[
                    'facc']]), value=-9999.0)
                ...
                for i in range(temp2['node_t'][nb_process].shape[0]):
                    ...
                    for j in range(temp2['node_t'][nb_process].shape[1]):
                        if temp2['node_t'][nb_process][i][j] > 0.0:
                            if i_obs < 0:
                                i_obs = i
                            index = np.array(np.where(abs(temp2['node_t'][
                                nb_process][i][j] - Generic_T) <= DT_obs))
                            while index.shape[1] != 1:
                                index = np.array(np.where(abs(temp2[
                                    'node_t'][nb_process][i][j] - Generic_T
                                    ) <= DT_obs))
                                if index.shape[1] < 1:
                            ...
                            for key in keys2:
                                if calc.check_na(temp2[key][nb_process][i, j]):
                                    concat_dict[key][i_mat, index] = min(params
                                        .valid_min_dA, params.valid_min_z)
                                    concat_dict[key][i_mat].mask[index] = True
                                else:
                                    concat_dict[key][i_mat, index] = temp2[key
                                        ][nb_process][i, j]
                            if i == i_obs:
                                for key in keys3:
                                    if calc.check_na(temp2[key][nb_process][j]
                                        ):
                                        concat_dict[key][nb_process][index] = min(
                                            params.valid_min_dA, params.valid_min_z
                                            )
                                        concat_dict[key][nb_process].mask[index
                                            ] = True
                                    else:
                                        concat_dict[key][nb_process][index
                                            ] = temp2[key][nb_process][j]
            q_monthly_mean_stack = np.stack(concat_dict['q_monthly_mean'])
            concat_dict['q_monthly_mean'] = np.nanmean(q_monthly_mean_stack,
                axis=0)
            masked_data3 = np.ma.masked_values(np.array([concat_dict[
            ...
            if run_flag:
                if new_process.output['valid']:
                    if new_process.output['valid_a5_sets'][nb_process]:
                        logging.info(
                            f'sic4dvar_set_run : writing results to output directory {output_dir}'
                            )
                        if nbr_reach > 1:
                            write_output(output_dir, param_dict,
                                reaches_ids[nb_process], new_process.output,
                                a5_counter)
                        if nbr_reach == 1:
                            write_output(output_dir, param_dict,
                                reaches_ids[nb_process], new_process.output)
                    ...
                    if param_dict['aws']:
                        if nbr_reach > 1:
                            write_output(param_dict['output_dir'],
                                param_dict, reaches_ids[nb_process],
                                new_process.output, -1, folder_name)
                        if nbr_reach == 1:
                            write_output(param_dict['output_dir'],
                                param_dict, reaches_ids[nb_process],
                                new_process.output, -1, folder_name)
                ...
                if param_dict['aws']:
                    if nbr_reach > 1:
                        write_output(param_dict['output_dir'], param_dict,
                            reaches_ids[nb_process], [], -1, folder_name,
                            max(dim_t))
                    if nbr_reach == 1:
                        write_output(param_dict['output_dir'], param_dict,
                            reaches_ids[nb_process], [], -1, folder_name,
        ...
        logging.info('Run time for current set is = %s.', str(t_set1 -
            t_reach0))
        close_logger()
    t1 = datetime.datetime.utcnow()
...
def accumulate_node_length(dist_out, node_length):
    ...
    dist_out_0 = scipy.stats.mode(np.abs(acc_node_len - dist_out), axis=None
        ).mode
...
def prepare_reaches(input_data, filtered_data, i):
    ...
    reach_info_list = []
    reach_node_x = []
    reach_node_x = accumulate_node_length(input_data['dist_out_all'][i],
        input_data['node_length_all'][i])
    reach_dict['node_z'] = np.empty((len(reach_node_x), len(input_data[
        'separate_reach_t'][i])))
    reach_dict['node_z'][:] = np.nan
    reach_dict['node_t'] = np.empty((len(reach_node_x), len(input_data[
        'separate_reach_t'][i])))
