...
from sic4dvar_functions.q558 import D as r
...
class b:
	...
	def plot(B,show=D,plot_title='',fig_width=15,fig_height=5,y_axis_title='',x_axis_title='',y_lim=C,x_max_lim=C,output_file=C):
...
if __name__=='__main__':
	B=b(width=[2e1,5e1,15e1,3e2,400],elevation=[5.,7.,10,15,20],sort=F,check_increasing=F,check_nan=F);c=b(width=[1.,1e1,2e1,3e1,40],elevation=[2.,2.,4,12,20]);B.compute();W=E.deepcopy(B);P=dict(y_lim=(0,25),x_max_lim=45e1);B.plot('0 big',**P);c.plot(plot_title='',**P);B.add_wet_bathy_from_other(c);B.plot(plot_title='',**P);B.add_dry_bathy_from_other(W);B.plot(plot_title='',**P);B=E.deepcopy(W);B.combine_with_other(c,preference=o);B.plot(plot_title='',**P);B=E.deepcopy(W);B.combine_with_other(c,preference='narrow');B.plot(plot_title='',**P);B.shrink_bottom_by_elevation(7.5);B.plot(plot_title='',**P);B.extend_bottom_by_elevation(4);B.plot(plot_title='',**P);W.shrink_bottom_by_width(100);W.plot(plot_title='',**P);B=b([992.46,1145.06],[20.15,20.91]);B.plot();B.extend_bottom_by_elevation(13.23);B.plot();x=A.array([J,4.,5,6,7,8,9,10,11])
	with A.printoptions(precision=2,suppress=D):0
	B=b(width=[1.82,106.07,106.79,214.37,215.3,216.],elevation=[4.85,4.9,5.,5.07,5.53,6.8],sort=F,check_increasing=F,check_nan=F,float_atol=.01);B.plot(y_lim=[2.,8.],plot_title='');B.extend_bottom_by_elevation(2.15);B.plot(y_lim=[2.,8.],plot_title='')