...
from sic4dvar_functions.q558 import D as s
from sic4dvar_functions.D771 import u
...
def N(w0_array,z0_array,cor_z=F,extrapolate_min=D,extrapolate_max=D,first_sweep=B.def_lsm_w_first_sweep,remove_bias_in_loop=B.def_lsm_w_rem_bias_in_loop,cs_float_atol=B.def_cs_float_atol,plot=J,plot_title=O,plot_colormap='YlOrBr',clean_run=D):
	...
	if b:
		...
		P.append(I.deepcopy(B/2));Q.append(I.deepcopy(C));R.append('-');L.append('black');T.append(2.5);S.append('final');U.append(O);V.append(4.);M.append(O);A7(xs=P,ys=Q,show=J,x_lim=(.9*A.nanmin(B/2),1.1*A.nanmax(B/2)),y_lim=(.9*A.nanmin(C),1.1*A.nanmax(C)),line_styles=R,line_widths=T,line_colors=L,line_labels=S,marker_styles=U,marker_sizes=V,marker_fill_colors=M,title=plot_title,x_axis_title='Half width (m)',y_axis_title='Elevation (m)',fig_width=15,fig_height=5,add_legend=J)
	return B,C
if __name__=='__main__':from pathlib import Path;from sic4dvar_low_cost.sic4dvar_functions.io.reader_swot_obs import get_vars_from_swot_nc as Q;S=Path('C:\\Users\\isadora.rezende\\PhD\\Datasets');C,T=Q(reach_ids=(56463000571,),swot_file_pattern=S/'SWOT'/'Nominal'/'netcdf'/'{}_SWOT.nc',node_vars=(H,K,'time'));E=C[L][H][0];G=C[L][K][0];M(cs_i_z0_array=E[0,:],cs_i_w0_array=G[0,:],max_iter=10,cor_z=F,clean_run=D,debug_mode=D,plot=J);N(w0_array=G,z0_array=E,plot=J,clean_run=D)